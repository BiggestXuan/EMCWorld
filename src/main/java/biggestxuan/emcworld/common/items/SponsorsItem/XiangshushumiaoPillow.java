...
public class XiangshushumiaoPillow extends EWItem {

    @OnlyIn(Dist.CLIENT)
    public void appendHoverText(ItemStack p_77624_1_, @Nullable World p_77624_2_, List<ITextComponent> p_77624_3_, ITooltipFlag p_77624_4_) {
        p_77624_3_.add(EMCWorld.tc("tooltip.emcworld.xiangshushumiao_pillow"));
        ClientPlayerEntity player = Minecraft.getInstance().player;
        if(player != null && !player.isDeadOrDying() && MathUtils.getSponsor(player) != null && MathUtils.getSponsor(player).equals(Sponsors.xiangshushumiao) && !EMCWorld.isOffline){
